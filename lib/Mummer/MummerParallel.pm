...
sub _mummerParallelInitialize {
	...
	$self->_baseDirectory( getcwd() . '/' ) unless defined $self->_baseDirectory;
}
...
sub run {
	...
	$self->logger->info( "INFO:\tStarting MummerParallel run\n\t"
		  . 'Mummer Directory: '
		  . $self->_mummerDirectory . "\n\t"
		  . 'Query File: '
		  . $self->_queryFile . "\n\t"
		  . 'Reference File: '
		  . $self->_referenceFile . "\n\t"
	...
	$self->_deltaFile( $self->_baseDirectory . $self->_p . '.delta' );
	...
	if ( $self->_forkNumber > 1 ) {
		...
		$ffSplitter->splitFastaFile( $self->_queryFile, $self->_forkNumber );
		...
		$runSettingsHash{'referenceFile'} = $self->_referenceFile;
		$runSettingsHash{'p'}             = $self->_baseDirectory . $self->_p;
		$mummerRun->runMummer(%runSettingsHash);
	}
}
...
sub _combineTempDeltaFiles {
	...
	#cannot use vanillaCombineFiles as headers to delta files are added to each subfile
	my $combiner = FileManipulation->new();
	my $combinedFH = IO::File->new( '>' . $self->_deltaFile ) or die 'Cannot open ' . $self->_deltaFile . "$!\n";
	$combiner->outputFilehandle($combinedFH);
	$combiner->combineDeltaFiles( $self->_arrayOfTempDeltaFiles, $self->_referenceFile, $self->_queryFile, 1 );    #flag destroys file
	$combinedFH->close();
}
