...
sub printPhylipFormat {
	...
	$self->printOut( scalar( keys %{ $self->tableHash } ),
	...
	for ( my $i = 1 ; $i <= scalar( keys %{ $self->tableHash } ) ; $i++ ) {
		...
		if (   defined $self->nameOrderArray
			&& defined $self->nameOrderArray->[ $i - 1 ] )
		{
	...
	$self->outputFilehandle( $self->phylipInfoFH )
	  ; #if phylipInfoFH is undefined, defualts to STDOUT (see FlexiblePrinter.pm)
	$self->printOut( '#Name Conversion Information', "\n" );

	for ( my $i = 1 ; $i <= scalar( keys %{ $self->tableHash } ) ; $i++ ) {
		$self->printOut( $i, ":\t", $names[ $i - 1 ], "\n" );
	}
}
