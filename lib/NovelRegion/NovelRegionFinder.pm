...
sub validateNovelSettings {
	...
	if (@_) {
		...
		foreach my $setting ( keys %{$settingsHashRef} ) {
			...
			$self->_novel_nucB( $validator->isAnInt($value) )                              if $setting eq 'novel_nucB';
			...
			$self->combinedReferenceFile( $validator->isAFilename($value) )                if $setting eq 'combinedReferenceFile';
			$self->_adjacentJoiningSize( $validator->isAnInt($value) )                     if $setting eq 'adjacentJoiningSize';
		}
