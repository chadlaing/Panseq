...
my @speciesList = $ftp->ls($directory . "a*/all_assembly_versions/*");
my @assemblyReports;
foreach my $s(@speciesList){
    if($s =~ m/_assembly_report/){
        push @assemblyReports, $s;
    }
}


foreach my $a(@assemblyReports){
    my $localFile;
    if($a =~ m/all_assembly_versions\/.*\/(.+)$/){
        $localFile = $localNcbiDirectory . $1;
    }
    else{
    ...
    $ftp->binary();
    $ftp->get($a, $localFile) or die ("download of $a failed\n" and next);
    $ftp->ascii();
}
